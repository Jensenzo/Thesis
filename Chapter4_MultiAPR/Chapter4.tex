...
p(j |y_c) = \argmax{j} \prod^C p(y_c|j,n_0,c),
\end{equation}

\begin{equation}\label{eq:MLdefinition2_c}
\log{\left( p(j |y_c)\right)} = \argmax{j} \sum^C \log{\left( p(y_c|j,n_0,c) \right)}.
\end{equation}
...
p(j|y) = \argmax{j} \prod^C \underset{n_0}{\max} p(y_c|j,n_o,c).
\end{equation}
