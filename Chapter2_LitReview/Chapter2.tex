...
The system proposed by the authors of \cite{Sugiyama2007} base their detection on the \gls{stft} and from that they focus on 7 factors:
\begin{enumerate}
...
\begin{equation}\label{eq:Sugiyama2013}
...
While the authors of \cite{Sugiyama2013} note their motivation behind said algorithm is linked to telecommunication networks no details about the algorithm in question's applicability in such a context is presented and while the paper does contain some objective evaluation these are compared to those of a standard magnitude based noise suppression algorithm by the same authors \cite{Sugiyama2002}. The authors present two examples with \gls{pesq} score improvements of 0.1 and 0.2 but propose cascading their approaches for further improvements. While these results are neither substantial nor well founded and with no comparison to the state of the art it is difficult to assess the actual performance of the algorithm proposed. This approach may be a novel approach to keyboard noise detection but few details about the implementation are presented and therefore questions regarding the algorithm's applicability in the real-time scenario are brought into question.


In \cite{Kauppinen2002} the authors present 3 methods for transient detection algorithms which are designed to detect non-Gaussian noise in realtime datastreams. Here the authors focus on detection of transients in musical signals. The methods presented are a derivative based method, a \gls{rms} based method and an \gls{ar} based method. The derivative based method simply proposes to look at the absolute value of the derivative of the time signal. The authors suggest that the 4th derivative gave the best results in their experience. The second method proposed by the authors is a \gls{rms} based detector. While the authors acknowledge that the \gls{rms} is not a particularly good basis for transient detection they note that significant improvements can be obtained by taking the absolute value of the 4th derivative of it. Lastly the authors of \cite{Kauppinen2002} propose to detect transients by considering the \gls{ar} error of the time signal. The authors suggest that the underlying musical signal be modeled with a filter order of 10 although refer to a much higher order of 1000 as an ideal filter order. It was found that the \gls{ar} based method performed the best with the tested data (drums, guitar, saxophone and speech). While these algorithms performed well for this given application it is noted that the application did not contain a specific noise source such as the keyboard typing application. It was although noted that the \gls{ar} was the best performing method and considering the computational restrictions for real time processing at the time of publication of \cite{Kauppinen2002} more advanced methods could be developed.

In \cite{Abramson2007} the authors propose a simultaneous estimation and detection approach with an \emph{a priori} \gls{snr} estimator, which facilitate suppression of transient noise. This approach uses a cost function which combines the detection and estimation error in the \gls{stft} domain. A speech coefficients detector and a corresponding estimator for the coefficient values are designed jointly with the aim of minimizing the combined Bayes risk.

To aid in transient noise detection the authors implement an \emph{a priori} \gls{snr} estimator. It is noted that while decision-directed estimators are widely used in similar scenarios, they are not suitable for transient noise environments, since a high energy transient noise may yield an instantaneous increase in \emph{a posteriori} \gls{snr}. This, in turn, would increase the spectral gain of the frequency bin and as a result the transient noise would not be attenuated to the correct degree. The authors propose to base their \emph{a priori} \gls{snr} estimator on the spectral variance where the variance for the underlying stationary noise can be estimated and the transient noise spectral variance is obtained through training. Since the \emph{a priori} \gls{snr} is highly dependent on the spectral variance the speech spectral variance is calculated whereafter the \emph{a priori} \gls{snr} is estimated. The authors show that this \gls{snr} estimate is lower and therefore enables further attenuation of transient noise.

In the paper the authors evaluate their approach against the \gls{stsa} approach and obtain favorable results for their approach. It is although noted that the transient noise introduced into the signals are simulated short bursts of high energy coloured noise. There are no indications that these transient noise events could be related to any acoustic phenomenon much less the explicit scenario of key strokes where events exhibit a far less constant or flat spectral response with multiple intertwined segments. The paper also omits any mention of computational concerns or applicability in a realtime scenarios and given the parameter choices such as a 256 tap overlapping Hamming filter for \gls{stft} decomposition it is doubtful if this approach would be feasible in a realtime communication scenario as proposed.
...
Specific keystroke removal systems were proposed in a couple of articles \cite{Subramanya2007}\linebreak[0]\cite{Sugiyama2007}\linebreak[0]\cite{Sugiyama2013}\cite{Seltzer2011Patent}. While some implementations propose real time implementations, none offer methods utilizing lookahead less than 10 or 20 ms and without the reliance of an \gls{os} cue.
% ------------------------------------------------------------------------
